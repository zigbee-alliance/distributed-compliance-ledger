...
const baseNocRootCertificates: object = { vid: 0 }

export const NocRootCertificates = {
  encode(message: NocRootCertificates, writer: Writer = Writer.create()): Writer {
    if (message.vid !== 0) {
  ...
  decode(input: Reader | Uint8Array, length?: number): NocRootCertificates {
    const reader = input instanceof Uint8Array ? new Reader(input) : input
    let end = length === undefined ? reader.len : reader.pos + length
    const message = { ...baseNocRootCertificates } as NocRootCertificates
    message.certs = []
  ...
  fromPartial(object: DeepPartial<NocRootCertificates>): NocRootCertificates {
    const message = { ...baseNocRootCertificates } as NocRootCertificates
    message.certs = []
    if (object.vid !== undefined && object.vid !== null) {
