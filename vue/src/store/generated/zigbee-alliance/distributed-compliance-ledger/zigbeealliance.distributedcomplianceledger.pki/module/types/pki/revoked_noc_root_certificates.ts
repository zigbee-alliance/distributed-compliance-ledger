...
const baseRevokedNocRootCertificates: object = { subject: '', subjectKeyId: '' }

export const RevokedNocRootCertificates = {
  encode(message: RevokedNocRootCertificates, writer: Writer = Writer.create()): Writer {
    if (message.subject !== '') {
  ...
  decode(input: Reader | Uint8Array, length?: number): RevokedNocRootCertificates {
    const reader = input instanceof Uint8Array ? new Reader(input) : input
    let end = length === undefined ? reader.len : reader.pos + length
    const message = { ...baseRevokedNocRootCertificates } as RevokedNocRootCertificates
    message.certs = []
  ...
  fromPartial(object: DeepPartial<RevokedNocRootCertificates>): RevokedNocRootCertificates {
    const message = { ...baseRevokedNocRootCertificates } as RevokedNocRootCertificates
    message.certs = []
    if (object.subject !== undefined && object.subject !== null) {
