...
const baseTestingResults: object = { vid: 0, pid: 0, softwareVersion: 0, softwareVersionString: '' }

export const TestingResults = {
  encode(message: TestingResults, writer: Writer = Writer.create()): Writer {
    if (message.vid !== 0) {
  ...
  decode(input: Reader | Uint8Array, length?: number): TestingResults {
    ...
    const message = { ...baseTestingResults } as TestingResults
    message.results = []
