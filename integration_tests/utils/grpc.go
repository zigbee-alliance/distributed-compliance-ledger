...
func SetupTest(t *testing.T, chainID string) (suite TestSuite) {
...
func (suite TestSuite) GetKeyring() keyring.Keyring {
	return suite.kr
}
...
func (suite *TestSuite) BroadcastTx(txBytes []byte) (*sdk.TxResponse, error) {
	...
	require.Equal(suite.t, uint32(0), grpcRes.TxResponse.Code, grpcRes.TxResponse)
...
func (suite *TestSuite) GetAccount(address sdk.AccAddress) (*dclauthtypes.Account, error) {
	grpcConn, err := GetGRPCConn()
	require.NoError(suite.t, err)
	defer grpcConn.Close()
	...
	res := accRes.GetAccount()
	return &res, nil
}

func (suite *TestSuite) GetAccounts() ([]dclauthtypes.Account, error) {
	grpcConn, err := GetGRPCConn()
	require.NoError(suite.t, err)
	defer grpcConn.Close()
	...
	// This creates a gRPC client to query the x/dclauth service.
	accClient := dclauthtypes.NewQueryClient(grpcConn)
	accRes, err := accClient.AccountAll(
		context.Background(),
	...
	return accRes.GetAccount(), nil
}

func (suite *TestSuite) GetProposedAccounts() ([]dclauthtypes.PendingAccount, error) {
	grpcConn, err := GetGRPCConn()
	require.NoError(suite.t, err)
	defer grpcConn.Close()
	...
	// This creates a gRPC client to query the x/dclauth service.
	accClient := dclauthtypes.NewQueryClient(grpcConn)
	accRes, err := accClient.PendingAccountAll(
		context.Background(),
	...
	return accRes.GetPendingAccount(), nil
}

func (suite *TestSuite) GetProposedAccountsToRevoke() ([]dclauthtypes.PendingAccountRevocation, error) {
	grpcConn, err := GetGRPCConn()
	require.NoError(suite.t, err)
	defer grpcConn.Close()
	...
	// This creates a gRPC client to query the x/dclauth service.
	accClient := dclauthtypes.NewQueryClient(grpcConn)
	accRes, err := accClient.PendingAccountRevocationAll(
		context.Background(),
	...
	return accRes.GetPendingAccountRevocation(), nil
}

func (suite *TestSuite) ProposeAddAccount(
	signer string,
...
func (suite *TestSuite) ApproveAddAccount(
...
func (suite *TestSuite) ProposeRevokeAccount(
	signer string, accAddr sdk.AccAddress, accnum uint64, sequence uint64,
) (*sdk.TxResponse, error) {
...
func (suite *TestSuite) ApproveRevokeAccount(
	signer string, accAddr sdk.AccAddress, accnum uint64, sequence uint64,
) (*sdk.TxResponse, error) {
