...
for node in "$GVN_NAME" "$VN_NAME"; do
    ...
    docker cp deployment/ansible/files/cosmovisor_preupgrade.sh "$node":"$DCL_USER_HOME"
    docker cp deployment/ansible/files/cosmovisor_start.sh "$node":"$DCL_USER_HOME"
