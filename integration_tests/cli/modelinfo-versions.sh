...
echo "Add Model with VID: $vid PID: $pid SV:$sv1 HV:$hv1"
result=$(echo "test1234" | dclcli tx modelinfo add-model --vid=$vid --pid=$pid --softwareVersion=$sv1 --hardwareVersion=$hv1 --supportURL="https://originalsupporturl.test" --productName="$productName" --description="Device Description" --sku="SKU12FS"  --softwareVersionString="1.0b123"   --hardwareVersionString="5.1.23"  --cdVersionNumber="32" --from $vendor_account --yes)
check_response "$result" "\"success\": true"
echo "$result"



echo "Get Model Versions with VID: $vid PID: $pid"
echo "dclcli query modelinfo model --vid=$vid --pid=$pid"
result=$(dclcli query modelinfo model-versions --vid=$vid --pid=$pid)
check_response "$result" "\"vid\": $vid"
check_response "$result" "\"pid\": $pid"
check_response "$result" "\"softwareVersion\": $sv"
check_response "$result" "\"hardwareVersion\": $hv"
check_response "$result" "\"softwareVersion\": $sv1"
check_response "$result" "\"hardwareVersion\": $hv1"
echo "$result"
