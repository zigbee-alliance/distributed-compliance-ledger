...
echo "dclcli tx modelinfo add-model --vid=$vid --pid=$pid --softwareVersion=$sv --hardwareVersion=$hv --productName="Device #1" --description="Device Description" --sku="SKU12FS"  --softwareVersionString="1.0b123"  --hardwareVersionString="5.1.23"  --cdVersionNumber="32" --from $vendor_account --yes"
...
echo "Add Second Testing Result for Model $vid PID: $pid SV: $sv HV: $hv"
second_testing_result="http://second.place.com"
second_test_date="2020-04-04T10:00:00Z"
result=$(echo "test1234" | dclcli tx compliancetest add-test-result --vid=$vid --pid=$pid --softwareVersion=$sv --hardwareVersion=$hv --test-result="$second_testing_result" --test-date=$second_test_date --from $second_test_house_account --yes)
check_response "$result" "\"success\": true"
echo "$result"

echo "Get Testing Result for Model with VID: ${vid} PID: ${pid}"
result=$(dclcli query compliancetest test-result --vid=$vid --pid=$pid --softwareVersion=$sv --hardwareVersion=$hv)
