...
result=$(echo "test1234" | dcld tx compliance certify-model --vid=$vid --pid=$pid --softwareVersion=$sv --cdVersionNumber=1 --softwareVersionString=1 --cdCertificateId=12345678910abcdefgh --certificationType=zigbee --specificationVersion="1.0" --certificationDate="2020-01-01T00:00:01Z" --from $zb_account --yes)
result=$(get_txn_result "$result")
echo "$result"
check_response "$result" "\"code\": 0"
