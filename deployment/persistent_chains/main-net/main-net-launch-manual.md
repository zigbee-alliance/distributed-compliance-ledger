...
6. **Update Empty Block Interval to 600s**.
...
7. **Wait until the CSA Mainnet infrastructure is up and running**
    ...
    7.1. Access DCL Web UI link (https://webui.dcl.csa-iot.org) and follow the ceremony procedure

8. **Run Sentry Node (skip this section if you are not running a sentry node)**
    ...
    8.2. Prepare `persistent_peers.txt` file 
    ...
    * Naming convention for `<node-name>` is `[company name]-[node type]-[sequence number]` e.g. `<CSA-SN-01>`
    ...
    8.4. Wait until catchup is finished: `./dcld status` returns `"catching_up": false`

9. **Sentry Node Deployment Verification (skip this section if you are not running a sentry node)**

    9.1. Check the account presence on the ledger: `./dcld query auth account --address="<address>"`.

    9.2. Check the cosmovisor service is running: `systemctl status cosmovisor`

    9.3. Check the node gets new blocks: `./dcld status`. Make sure that `result.sync_info.latest_block_height` is increasing over the time (once in about 5 sec).

10. **Run Validator Node**

    10.1. Wait until your NodeAdmin account has been apporoved by CSA

    10.2. Download genesis

    ```bash
    curl -L -O https://raw.githubusercontent.com/zigbee-alliance/distributed-compliance-ledger/master/deployment/persistent_chains/main-net/genesis.json
    ```

    10.3. Prepare `persistent_peers.txt` file
