...
func TestListChildCertificates(t *testing.T) {
	net, objs := networkWithChildCertificatesObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListChildCertificates(), args)
			require.NoError(t, err)
			var resp types.QueryAllChildCertificatesResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
