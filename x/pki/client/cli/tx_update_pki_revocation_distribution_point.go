...
func CmdUpdatePkiRevocationDistributionPoint() *cobra.Command {
	...
	cmd.Flags().StringVarP(&label, FlagLabel, FlagLabelShortcut, "", "A label to disambiguate multiple revocation information partitions of a particular issuer. Maximum 64 characters.")
	cmd.Flags().StringVarP(&crlSignerCertificate, FlagCertificate, FlagCertificateShortcut, "", "The issuer certificate whose revocation information is provided in the distribution point entry, encoded in X.509v3 PEM format. The corresponding CLI parameter can contain either a PEM string or a path to a file containing the data. Please note that if crlSignerCertificate is a delegated certificate by a PAI, the delegator certificate must be provided using the `crlSignerDelegator` field. Maximum size: 2 KiB.")
	...
	cmd.Flags().StringVar(&dataURL, FlagDataURL, "", "The URL where to obtain the information in the format indicated by the RevocationType field. Must start with either http or https. Maximum 256 characters; must be unique for all pairs of VendorID and IssuerSubjectKeyID. In release builds the URL must be reachable.")
	...
	cmd.Flags().StringVar(&dataDigest, FlagDataDigest, "", "Digest of the entire contents of the associated file downloaded from the DataURL. Must be omitted if RevocationType is 1. Must be provided if and only if the DataFileSize field is present. Maximum 128 characters.")
