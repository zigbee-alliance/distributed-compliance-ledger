...
func TestHandler_RevokeNocX509Cert_RevokeDefault(t *testing.T) {
	...
	addNocX509Cert = types.NewMsgAddNocX509IcaCert(accAddress.String(), testconstants.NocCert1Copy, testconstants.CertSchemaVersion)
	_, err = setup.Handler(setup.Ctx, addNocX509Cert)
	require.NoError(t, err)

	// add the NOC leaf certificate
	addNocX509Cert = types.NewMsgAddNocX509IcaCert(accAddress.String(), testconstants.NocLeafCert1, testconstants.CertSchemaVersion)
	_, err = setup.Handler(setup.Ctx, addNocX509Cert)
	require.NoError(t, err)
	...
	nocCerts, err := queryNocCertificatesByVid(setup, testconstants.Vid)
	require.NoError(t, err)
...
func TestHandler_RevokeNocX509Cert_RevokeWithChild(t *testing.T) {
	...
	_, err = queryNocCertificates(setup, testconstants.NocCert1Subject, testconstants.NocCert1CopySubjectKeyID)
	require.Error(t, err)
	require.Equal(t, codes.NotFound, status.Code(err))
	...
	// query noc certificate by VID
	_, err = queryNocCertificatesByVid(setup, testconstants.Vid)
	require.Error(t, err)
	require.Equal(t, codes.NotFound, status.Code(err))
	...
	revokedLeafCerts, err := queryRevokedNocIcaCertificates(setup, testconstants.NocLeafCert1Subject, testconstants.NocLeafCert1SubjectKeyID)
	require.NoError(t, err)
