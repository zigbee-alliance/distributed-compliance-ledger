...
func (k Keeper) SetTestingResults(ctx sdk.Context, testingResult types.TestingResults) {
	...
	store.Set(types.GetTestingResultsKey(
		testingResult.VID, testingResult.PID, testingResult.SoftwareVersion,
		testingResult.HardwareVersion), k.cdc.MustMarshalBinaryBare(testingResult))
}
...
func (k Keeper) AddTestingResult(ctx sdk.Context, testingResult types.TestingResult) {
	testingResults := k.GetTestingResults(ctx, testingResult.VID, testingResult.PID,
		testingResult.SoftwareVersion, testingResult.HardwareVersion)
...
func (k Keeper) IsTestingResultsPresents(ctx sdk.Context, vid uint16, pid uint16,
...
func (k Keeper) IterateTestingResults(ctx sdk.Context,
	process func(info types.TestingResults) (stop bool)) {
	store := ctx.KVStore(k.storeKey)
