...
import (
	...
	govutils "github.com/zigbee-alliance/distributed-compliance-ledger/x/gov_old/client/utils"
...
Example:
$ %s tx gov_old submit-proposal --proposal="path/to/proposal.json" --from mykey
...
$ %s tx gov_old submit-proposal --title="Test Proposal" --description="My awesome proposal" --type="Text" --deposit="10test" --from mykey
`,
...
find the proposal-id by running "%s query gov_old proposals".

Example:
$ %s tx gov_old deposit 1 10stake --from mykey
`,
				version.ClientName, version.ClientName,
			),
		),
...
Example:
$ %s tx gov_old vote 1 yes --from mykey
`,
				version.ClientName, version.ClientName,
			),
		),
