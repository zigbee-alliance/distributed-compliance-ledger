...
func InitGenesis(ctx sdk.Context, k keeper.Keeper, genState types.GenesisState) (res []abci.ValidatorUpdate) {
	...
	// Set all the proposedDisableValidator
	for _, elem := range genState.ProposedDisableValidatorList {
		k.SetProposedDisableValidator(ctx, elem)
	}
	// Set all the disabledValidator
	for _, elem := range genState.DisabledValidatorList {
		k.SetDisabledValidator(ctx, elem)
	}
	// Set all the rejectedNode
	for _, elem := range genState.RejectedValidatorList {
...
func ExportGenesis(ctx sdk.Context, k keeper.Keeper) *types.GenesisState {
	...
	genesis.RejectedValidatorList = k.GetAllRejectedDisableValidator(ctx)
