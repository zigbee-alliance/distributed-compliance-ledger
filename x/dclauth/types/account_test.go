...
func TestPendingAccountRevocation_Validate(t *testing.T) {
	type fields struct {
		Address     string
		Revocations []*Grant
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			revoc := PendingAccountRevocation{
				Address:     tt.fields.Address,
				Revocations: tt.fields.Revocations,
			}
			if err := revoc.Validate(); (err != nil) != tt.wantErr {
				t.Errorf("PendingAccountRevocation.Validate() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}

func TestPendingAccountRevocation_HasRevocationFrom(t *testing.T) {
	type fields struct {
		Address     string
		Revocations []*Grant
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := revoc.HasRevocationFrom(tt.args.address); got != tt.want {
				t.Errorf("PendingAccountRevocation.HasRevocationFrom() = %v, want %v", got, tt.want)
			}
		})
	}
}
