...
starport scaffold --modele dclupgrade message RejectUpgrade name:string
...
# Change `plan` field type to cosmos.upgrade.v1beta1.Plan after scaffolding.
# Then change index to plan.name field and remove redundant name field.
starport scaffold --module dclupgrade map RejectedUpgrade plan:string creator:string approvals:array.string --index name:string --no-message