...
5. All certificate-add transactions enforce the Matter R1.5 structural profile per certificate type: §6.2.2.5 for PAA, §6.2.2.4 for PAI, §6.2.2.3 for DAC, and §6.5.12 for RCAC / ICAC / NOC. The DA-chain add transaction (used for both PAI and DAC) and the NOC-chain non-root add transaction (used for both ICAC and NOC) dispatch by the cert's BasicConstraints `cA` flag so each certificate is checked under the right profile. A summary of the per-profile structural rules — version v3, ecdsa-with-SHA256 on prime256v1, BasicConstraints critical with the correct `cA`, KeyUsage critical with profile-specific bits, Subject Key Identifier present, Authority Key Identifier present on non-self-signed CAs, no ExtendedKeyUsage on RCAC / ICAC, immediate-issuer VID / PID consistency in the DA chain, and at most one matter-vid / matter-pid per Distinguished Name — is documented at the top of the [X509 PKI section](transactions/pki.md).
