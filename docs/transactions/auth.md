...
The total number of accounts with the `Trustee` role is limited to **12**. Once this limit is reached,
no new account can be proposed until a Trustee account is removed.
...
- Parameters:
  ...
  - roles: `array<string>` - the list of roles, comma-separated, assigning to the account. Supported roles: `Vendor`, `CertificationCenter`, `Trustee`, `NodeAdmin`, `VendorAdmin`.
...
- Number of required approvals:
  - greater than or equal 2/3 of Trustees for account roles: `CertificationCenter`, `Trustee`, `NodeAdmin`, `VendorAdmin` (proposal by a Trustee is also counted as an approval)
...
- Number of required rejects:
  - greater than 1/3 of Trustees for account roles: `CertificationCenter`, `Trustee`, `NodeAdmin`, `VendorAdmin` (proposal by a Trustee is also counted as an approval)
  - greater than or equal 2/3 of Trustees for account role: `Vendor` (proposal by a Trustee is also counted as an approval)
- CLI command:
  - `dcld tx auth reject-add-account --address=<bech32 encoded string> --from=<account>`
...
- Parameters:
  - address: `string` - account address; Bech32 encoded
- CLI command:
  - `dcld query auth account --address <bech32 encoded string>`
- REST API:
...
- Parameters:
  - address: `string` - account address; Bech32 encoded
- CLI command:
  - `dcld query auth proposed-account --address <bech32 encoded string>`
- REST API:
...
- Parameters:
  - address: `string` - account address; Bech32 encoded
- CLI command:
  - `dcld query auth proposed-account-to-revoke --address <bech32 encoded string>`
- REST API:
...
- Parameters:
  - address: `string` - account address; Bech32 encoded
- CLI command:
  - `dcld query auth revoked-account --address <bech32 encoded string>`
- REST API:
