...
1) Catchup from genesis:
  ...
  - Cons:
    ...
    - Probably error-prone (if at least one migration has bug catchup fails)
...
- Steps:
  - Enable state sync in `$HOME/.dcl/config/config.toml`:
    ...
    trust_height = 429539
    trust_hash = "F5D69C5BAFB69388980F6550CB68362CCF10491F8A98A96AB9008BBC186D48D4"
    trust_period = "168h0m0s"
    ```
  - Provide `seeds` or `persistent_peers` in `$HOME/.dcl/config/config.toml`
  - Run the new node
